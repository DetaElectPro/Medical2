...
class RequestSpecialistsAPIController extends AppBaseController
{
    ...
    public function show($id)
    {
        ...
        $requestSpecialists = $this->requestSpecialistsRepository->findWith($id, ['specialties.medical', 'user', 'acceptRequest.doctor.employ']);
    ...
    {
        $user = auth('api')->user();
        return $request = RequestSpecialists::where('user_id', $user->id)
    ...
    {
        $user = auth('api')->user();
        return $request = RequestSpecialists::where('doctor_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->get();
    }
